...
const listToBoard = list => {
  ...
  return list.map(resolveCell).reduce(writeToBoard, board);
};
...
const printBoard = (list, clues) => {
  ...
  const leftClues = clues
    .slice(3 * N, 4 * N)
    .map(zerosToSpaces)
    .reverse();
  const bottomClues = clues
    .slice(2 * N, 3 * N)
    .map(zerosToSpaces)
    .reverse();
...
const setConstraints = (list, idx, constraints) => {
  console.assert(
    Array.isArray(constraints) === true,
    'setConstraints passed a non-array value'
  );
...
const adjacentsFromClueIndex = index => {
  ...
  if (index < N) {
    const max = N * N - 1;
    return inclusiveRange(0, max).filter(x => x % N === index);
  }
  // right clues
